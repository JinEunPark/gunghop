...
export function useKakaoShare() {
  ...
  async function waitForSdk(timeoutMs = 6000): Promise<void> {
    ...
    while (Date.now() - start < timeoutMs) {
      if (typeof window !== 'undefined' && typeof window.Kakao?.init === 'function') return
  ...
  async function shareCardImage(opts: {
    ...
    try {
      const [, blob] = await Promise.all([waitForSdk(), captureCardBlob(opts.cardEl)])
      const K = getKakao()

      const file = new File([blob], 'nyangsang-result.png', { type: 'image/png' })
